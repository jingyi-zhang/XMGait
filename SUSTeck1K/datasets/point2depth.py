...
def pcd2depth(img_groups: Tuple, output_path: Path, img_size: int = 64, verbose: bool = False, dataset='CASIAB') -> None:
    ...
    for indx in range(len(joint_locations)-1):
        ...
        #     continue
        try:
            lidar_to_2d_front_view(warp, v_res=VRES, h_res=HRES, v_fov=VFOV, val="depth",
    ...
    for sid in tqdm(sorted(os.listdir(input_path))):
        for seq in os.listdir(os.path.join(input_path,sid)):
            for view in os.listdir(os.path.join(input_path,sid,seq)):
                joint_pkl_path = os.path.join(input_path, sid, seq, view,f'25-{view}-Joints-Location.pkl')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_path', type=str, help='Root path of raw dataset.')
    parser.add_argument('-o', '--output_path', type=str, help='Output path of pickled dataset.')
