...
class BaseModel(MetaModel, nn.Module):
    ...
    def inputs_pretreament(self, inputs):
        ...
        """
        seqs_batch, labs_batch, typs_batch, vies_batch, height_batch, gender_batch, seqL_batch = inputs
        ...
        height = list2var(height_batch)
        gender = list2var(gender_batch).long()
        ...
        if seqL is not None:
            ...
            ipts = [seqs[_][:, :seqL_sum] for _ in range(1,len(seqs))]
            ipts.insert(0,seqs[0])
        else:
            ipts = seqs
        del seqs
        return ipts, labs, typs, vies, height, gender, seqL
