...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-05', 'nm-06'], 'BG': ['bg-01', 'bg-02'], 'CL': ['cl-01', 'cl-02']},
                      ...
                      'Xmu_gait': {'close2':['2'], 'close3':['3'], 'close4':['4'], 'overall':['2','3','4']}
                      # 'Xmu_gait': {'fast':['Fast'], 'slow':['Slow'], 'overall':['Fast','Slow']}
                      }
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        for (v1, probe_view) in enumerate(view_list):
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
                view, probe_view)
            pseq_mask = pseq_mask if 'SUSTech1K' or 'Xmu_gait' not in dataset else np.any(np.asarray(
            ...
            for (v2, gallery_view) in enumerate(view_list):
                gseq_mask = np.isin(seq_type, gallery_seq_dict[dataset]) & np.isin(
                    view, [gallery_view])
                gseq_mask = gseq_mask if 'SUSTech1K' or 'Xmu_gait' not in dataset  else np.any(np.asarray(
...
def evaluate_indoor_dataset(data, dataset, metric='euc', cross_view_gallery=False):
    ...
    if dataset not in ('CASIA-B', 'OUMVLP', 'CASIA-E', 'SUSTech1K','Xmu_gait'):
